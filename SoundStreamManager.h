...
namespace AudioManager
{
	...
	public:
		audioID_t Load(const std::string& path, bool looping = false, float volume = 100, float pitch = 1)
		{
			...
			SoundStreamData* streamData = new SoundStreamData();
			streamData->path = path;
			streamData->looping = looping;
			streamData->volume = volume;
			streamData->pitch = pitch;
			streamData->ID = GenerateID();
			streamData->audio = MakeSoundStream(streamData);
			streamData->duration = streamData->audio->getDuration().asMilliseconds();
			audios[streamData->ID] = streamData;
