...
namespace AudioManager
{
	...
	protected:
		...
		std::unordered_map<std::string, audioID_t> audio_name_id_cr;
	...
	public:
		...
		void Unload(audioID_t audioID)
		{
			...
			auto result_cr = audio_name_id_cr.find(GetAudioData(audioID)->name);

			audio_name_id_cr.erase(result_cr);
		...
		//** getter functions **
		audioID_t GetAudioIdByName(std::string name) {
			return audio_name_id_cr[name];
		}
